...
`direct_search.py` compares the default Markdown response with explicit JSON. The other direct scripts opt into JSON when they normalize fields into bounded summaries. Agent examples use the default compact Markdown response.
...
Set `SERPAPI_API_KEY` plus the model provider key used by the script, then run
the published package with the matching optional extra from the project root:
...
Open [SDK examples](../docs/sdk-examples/index.html) for installation and complete
