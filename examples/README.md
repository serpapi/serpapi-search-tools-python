...
| `langchain_grok.py` | Local discovery | `langchain` | Maps and web |
| `langgraph_openai.py` | Stateful research | `langgraph` | Web and news |
...
| `llamaindex_openai.py` | Destination research | `llamaindex` | Travel explore and web |
...
uv run --extra langchain python examples/langchain_grok.py
...
`gemini-3.5-flash`, and `grok-4.5`. Override the corresponding model
environment variable when needed.

The two multi-tool OpenAI Agents scenarios use `_logging_client.py` to log safe
request metadata and return only the first three items from the engine's primary
result family to the model. This keeps multi-tool prompts within ordinary token
limits while leaving the package's default full-response behavior unchanged.
